...
(:        $server := if($joiner eq xdmp:hostname()) then $server else
	...
	$ssl-certificate := xdmp:base64-encode(xdmp:read-cluster-config-file("hosts.xml")//ho:host[contains(ho:host-name,$joiner)]/ho:ssl-certificate/text()),
	$host-id := xdmp:read-cluster-config-file("hosts.xml")//ho:host[contains(ho:host-name,$joiner)]/ho:host-id/text(),
	$get-grp-info := xdmp:http-get( concat($protocol,"://",$server,":",$port,"/accept-joiner.xqy?joiner=",$joiner),
	...
	$addHost := xdmp:http-get( concat($protocol,"://",$server,":",$port,"/accept-joiner-go.xqy?joiner=",$joiner,"&port=",$port,"&bind=",$bind, "&connect=",$connect, "&protocol=", $protocol, "&ssl-certificate=", $ssl-certificate, "&host-id=", $host-id, "&joiner-host-id=", $host-id, "&server=", $server, "&group=", $group, "&foreign-bind=", $foreign-bind, "&foreign-connect=", $foreign-connect),
                <options xmlns="xdmp:http">
        	<authentication method="digest">
         		<username>{$user}</username>
         		<password>{$pass}</password>
