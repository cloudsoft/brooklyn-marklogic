...
let $node-assignments := if ($hostname ne "" )
     then $ASSIGNMENTS [ an:host = $hostid and an:forest-id = $database/db:forests/db:forest-id ]
     else $ASSIGNMENTS [ an:forest-id = $database/db:forests/db:forest-id ]

let $forest-ids  as xs:unsignedLong* := $node-assignments/an:forest-id
let $forest-status := for $f in $forest-ids return xdmp:forest-status($f)
...
let $query-read-ms := xs:dayTimeDuration($query-read-time) div xs:dayTimeDuration("PT0.001S")
...
$replication-send-bytes,' ',$replication-send-rate,' ',$replication-send-ms,' ',$replication-send-load,codepoints-to-string(10));
