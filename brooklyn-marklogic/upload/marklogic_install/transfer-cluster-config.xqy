...
(:	$joiner := if($server eq xdmp:hostname()) then $joiner else
		   substring-before($joiner,"."), :)
(:        $server := if($server eq xdmp:hostname()) then $server else
	...
	$copyConfigFiles := xdmp:http-post( concat( $protocol, "://",$joiner,":",$port,"/receive-config-go.xqy"),
		...
		<data> { concat( "assignmentsFile=", $assignmentsFile, "&databasesFile=", $databasesFile, "&groupsFile=", $groupsFile, "&hostsFile=", $hostsFile, "&mimetypesFile=", $mimetypesFile, "&protocol=", $protocol , "&clustersFile=" , $clustersFile )
		}
		</data>
     		</options>)
(:
